define([
    ...
    function Engine() {
        ...
        self.choose5 = null;
        self.choose6 = null;
        self.roundnumber = null;
        self.totalincome = null;
        self.timezone = null;
        ...
        self.timer_minutes = null;
        self.timer_seconds = null;
        ...
        self.win_payrate;
        self.play_payrate;
        self.jackpot_payrate;
        self.jackpot_payamount;
        self.loto_price;
        ...
        self.ws.on('game_started', function(bets) {
            ...
            // let bets = info.bets;
            // self.number1 = info.number1;
            // self.number2 = info.number2;
            // self.number3 = info.number3;
            // self.number4 = info.number4;
            // self.number5 = info.number5;
            // self.number6 = info.number6;            
        ...
        self.ws.on('game_crash', function(data) {
            ...
            self.timer_minutes = data.timer_minutes;
            self.timer_seconds = data.timer_seconds;
        ...
        self.ws.on('game_starting', function(info) {
            ...
            self.number6 = info.number6;            
            self.gameId = info.game_id;
            self.roundnumber = info.roundnumber;
        ...
        self.ws.on('timer', function(info) {
            //console.log(info.min, info.sec)
            self.min =  info.min;            
            self.sec = info.sec;


            self.roundnumber = info.roundnumber;
            self.jackpot_payamount = info.jackpot_payamount;
            self.loto_price = info.loto_price;

            self.trigger('timer', info);
        });          
        ...
        self.ws.on('buylotto', function(data) {
            if (self.username === data.username) {
                self.placingBet = false;
                self.nextBetAmount = null;
                self.nextAutoCashout = null;
        ...
        self.ws.on('cashed_out', function(resp) {
            ...
            //self.playerInfo[resp.username].stopped_at = resp.stopped_at;
             self.playerInfo[resp.username].balance = resp.balance;
        ...
        self.ws.on('set_sum', function(resp) {
           //Add the cashout percentage of each user at cash out           
            self.totalincome = resp.totalincome;
           
            self.trigger('set_sum', resp);
        });
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: window.DEV_OTT? window.DEV_OTT : ott },
                    function(err, resp) {
                        ...
                        self.mynum1 = resp.mynum1;
                        self.mynum2 = resp.mynum2;
                        self.mynum3 = resp.mynum3;
                        self.mynum4 = resp.mynum4;
                        self.mynum5 = resp.mynum5;
                        self.mynum6 = resp.mynum6;

                        self.ticket_fee = resp.ticket_fee;
                        self.win_payrate = resp.win_payrate;
                        self.play_payrate = resp.play_payrate;
                        self.jackpot_payrate = resp.jackpot_payrate;
                        self.jackpot_payamount = resp.jackpot_payamount;
                        self.loto_price = resp.loto_price;
                        ...
                        self.playHistory = resp.play_history;
                        self.roundnumber = resp.roundnumber;
                        self.timezone = resp.timezone;
    ...
    //};
     ...
     /* Save My Numbers */
    Engine.prototype.savemynumbers = function(choose1,choose2,choose3,choose4,choose5,choose6) {
        var self = this;

        this.ws.emit('savemynumbers', choose1,choose2,choose3,choose4,choose5,choose6, function(error) {

            if (error) {
                console.warn('savemynumbers error: ', error);
        ...
        self.trigger('savemynumbers');
    };

     /** Buy LOTO */
    Engine.prototype.buyloto = function(ticket,ticket_fee,choose1,choose2,choose3,choose4,choose5,choose6,round,userid) {
        var self = this;
        ...
        var playInfo = {
                ...
                choose6: choose6,
                roundnumber: round,
                player_info: self.username,
                user_id: userid, 
            };

            //Add the current game info to the game history and if the game history is larger than 40 remove one element
            if (self.playHistory.length >= 40)
                self.playHistory.pop();
            self.playHistory.unshift(playInfo);

        this.ws.emit('buy_loto', ticket,ticket_fee,choose1,choose2,choose3,choose4,choose5,choose6,round, function(error) {

            if (error) {
                console.warn('buy_loto error: ', error);
    ...
    Engine.prototype.stopgame = function() {
        this.ws.emit('stopgame', function(error) {
            if (error) {
                console.warn('stopgame error: ', error);
                if (error !== 'GAME_IN_PROGRESS' && error !== 'ALREADY_PLACED_BET') {
                    alert('There was an error, please reload the window: ' + error);
                }
                if (callback)
    ...
    Engine.prototype.presetnextround = function(nextrate) {
        //console.log('in engine', nextrate)
        this.ws.emit('presetnextround', nextrate,  function(error) {            
            if (error) {
                console.warn('presetnextround error: ', error);
                if (error !== 'GAME_IN_PROGRESS' && error !== 'ALREADY_PLACED_BET') {
                    alert('There was an error, please reload the window: ' + error);
                }
                if (callback)
    ...
    AppDispatcher.register(function(payload) {
        ...
        switch(action.actionType) {
            ...
            case AppConstants.ActionTypes.SAVE_MyNUMBERS:
                EngineSingleton.savemynumbers(action.choose1, action.choose2, action.choose3,action.choose4,action.choose5,action.choose6);
                break;

             case AppConstants.ActionTypes.BUY_LOTO:
                EngineSingleton.buyloto(action.ticket, action.ticket_fee, action.choose1, action.choose2, action.choose3,action.choose4,action.choose5,action.choose6, action.round, action.userid);
                break;
            ...
            case AppConstants.ActionTypes.presetnextround:
                EngineSingleton.presetnextround(action.nextrate);
                break;         
